...
index_data = pd.read_csv('D:/Wind/data/MarketDay/000001.SZ.MarketDay.csv', parse_dates=['MarketTime'])
# 保留这几个需要的字段：'MarketTime', 'HIGH', 'LOW', 'CLOSE', 'PCT_CHG'
index_data = index_data[['MarketTime', 'HIGH', 'LOW', 'CLOSE', 'PCT_CHG']]
# 对数据按照【date】交易日期从小到大排序
index_data.sort('MarketTime', inplace=True)
...
# 取1992年之后的数据，排出较早的数据
index_data = index_data[index_data['MarketTime'] >= pd.to_datetime('19930101')]

# 当仓位为1时，买入上证指数，当仓位为0时，空仓。计算从19920101至今的资金指数
index_data['资金指数'] = (index_data['PCT_CHG'] * index_data['当天的仓位'] + 1.0).cumprod()
initial_idx = index_data.iloc[0]['CLOSE'] / (1 + index_data.iloc[0]['PCT_CHG'])
index_data['资金指数'] *= initial_idx

# 输出数据到指定文件
index_data[['MarketTime', 'HIGH', 'LOW', 'CLOSE', 'PCT_CHG', '最近N1个交易日的最高点',
            '最近N2个交易日的最低点', '当天的仓位', '资金指数']].to_csv('D:/Wind/data/turtle.csv', index=False, encoding='gbk')


# ==========计算每年指数的收益以及海龟交易法则的收益
index_data['海龟法则每日涨跌幅'] = index_data['PCT_CHG'] * index_data['当天的仓位']
year_rtn = index_data.set_index('MarketTime')[['PCT_CHG', '海龟法则每日涨跌幅']].\
